	...
	public function jpb_display_Settings() {
		...
		if ( $hs_app_id != "") {//recently added
			if ($hs_testing == "jpb_Tracker_testing") {
				wp_enqueue_script( 'tags-mj-ninja', 'https://tags.mediajel.ninja/?appId='.$hs_app_id.'&environment='.$hs_cart, '', $this->version, false );
				//echo '<script src="https://tags.mediajel.ninja/?appId='.$hs_app_id.'"></script>';
			}
			else {
				wp_enqueue_script( 'tags-cnna', 'https://tags.cnna.io/?appId='.$hs_app_id.'&environment='.$hs_cart, '', $this->version, false );
